...
print(labels)
```
Reads class names from the obj.names file and prints them. <br>
Output:
```css
['license_plate']
```
...
```
Gets and prints all layer names in the YOLO model.<br>
Output:
```css
...
```
Identify the names of the output layers required by the YOLO algorithm. <br>
Output:
```css
[327 353 379]
...
plt.show()
```
Reads the input image and displays its shape. Then, it displays the image. <br>
Output shape:
```css
(1280, 1024, 3)
```
...
print(blob.shape)
```
Creates a blob from the image to pass to the YOLO model and prints the shapes. <br>
Output:
```css
(1280, 1024, 3)
(1, 3, 416, 416)
...
```
Prints the types of the outputs.<br> 
Output:
```css
<class 'tuple'>
<class 'numpy.ndarray'>
...
print(colours[0])
```
Generates random colors for bounding boxes. <br>
Output:
```css
(1, 3)
[102 220 225]
...
```
Prints and saves detected labels. <br>
Output:
```css
license_plate
license_plate
